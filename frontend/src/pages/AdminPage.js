...
const UserManagementCard = ({ users }) => (
    <div className="bg-slate-100 p-6 rounded-lg shadow-md w-full">
        ...
        <ul className="space-y-2 max-h-60 overflow-y-auto">
            {users.map((user) => (
                <li key={user.email} className="flex justify-between items-center p-2 rounded-md bg-gray-50">
                    <div>
                        <p className="font-semibold">{user.email}</p>
                        <p className={`text-sm font-bold ${user.role === 'admin' ? 'text-red-600' : 'text-gray-500'}`}>{user.role}</p>
                    </div>
                    <button className="text-blue-600 hover:underline text-sm">Editar</button>
                </li>
            ))}
             {users.length === 0 && <li className="text-gray-500">Nenhum usuário encontrado.</li>}
        </ul>
    </div>
);
    ...
    return (
        ...
        >
            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto p-4 md:p-8 pt-24 md:pt-32">
                <h1 className="text-4xl font-bold text-white mb-8 text-center">Painel do Administrador</h1>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                         <div className="bg-slate-100 p-6 rounded-lg shadow-md w-full">
                            ...
                            <div className="space-y-4">
                               <Link to="/listar-formularios" className="w-full block text-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition-colors">
                                    Visualizar Formulários
                                </Link>
                                 <Link to="/relatorios-admin" className="w-full block text-center bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-lg transition-colors">
